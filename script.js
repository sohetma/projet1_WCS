...
var nbStudents = 18; // number of students in the class
var iteration = 0;
arrayStudents = [
        {
...
var changeStudent = () => {
  ...
  var actualStudent = getRandomInt(nbStudents);
  var picRandom = document.getElementById('pictureRandom');
  if(!picRandom){
    console.log(picRandom);
  }
  else{
    picRandom.src = `${arrayStudents[iteration].pictureStudent}`;
    picRandom.alt = `Portrait of ${arrayStudents[iteration].name}`;
  }
  iteration ++;
 //textStudents = textStudent(actualStudent);
 //console.log(textStudents);
 //return textStudents;
 //textStudent = [`<span> @wilder :~/${arrayStudents[actualStudent].name}$</span> I'm ${arrayStudents[actualStudent].age} and really happy to code in Wild code school <BR /> <BR /> <span>@wilder :~/Pierre$</span> Cool ! <BR />  <BR /> <span>@wilder :~/${arrayStudents[actualStudent].name}$</span>  TEST`];
}
...
var textStudent = (n) => {
  let j = 0;
  while(j < n){
    textStudents.push(`<span> @wilder :~/${arrayStudents[j].name}$</span> I'm ${arrayStudents[j].age} and really happy to code in Wild code school <BR /> <BR /> <span>@wilder :~/Pierre$</span> Cool ! <BR />  <BR /> <span>@wilder :~/${arrayStudents[j].name}$</span>  Code is better than  ${arrayStudents[j].job}`);
    j++;
  }
return textStudents;
}
...
textStudents = textStudent(nbStudents);
...
// Animation Header Hero
setTimeout(animIntro, 1500);
setInterval(changeStudent, 10000);
//myFunction2();
