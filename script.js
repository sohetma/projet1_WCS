...
var textStudent = (n) => {
  let j = 0;
  while(j < n){
    textStudents.push(`<span> @wilder :~/${arrayStudents[j].name}$</span> I'm ${arrayStudents[j].age} and really happy to code in Wild code school <BR /> <BR /> <span>@wilder :~/Pierre$</span> Cool ! <BR />  <BR /> <span>@wilder :~/${arrayStudents[j].name}$</span>  TEST`);
    j++;
  }
return textStudents;
}
